...
class phpMyDumper {
	...
	*/
	function setOutputFile($filepath) {
		if ( $this->isWritten )
			return false;
		$this->_html.= "Creating file '".$filepath."': ";
	...
	*/
	function getDatabaseStructure() {
		...
		while ( $record = @mysql_fetch_row($records) ) {
			$this->_html.= "Eksport struktury tabeli '".$record[0]."': ";
			$this->getTableStructure($record[0]);
			$this->_html.= " UKOŃCZONO!\n";
		}
		return true;
 	}
	...
	*/
	function getDatabaseData($hexValue = true) {
		...
		if ( @mysql_num_rows($records) == 0 )
			return false;
		while ( $record = @mysql_fetch_row($records) ) {
			if ($this->filename) $this->_html.= "Eksport danych z tabeli '".$record[0]."': ";
			$this->getTableData($record[0],$hexValue);
			if ($this->filename) $this->_html.= " UKOŃCZONO!\n";
		}
  	}
	...
	*/
	function getDatabaseStructureData($hexValue = true){
		...
		while ( $record = @mysql_fetch_row($records) ) {
			if ( $this->createTable) {
				if ($this->filename) $this->_html.= "Exporting table structure for '".$record[0]."': ";
				$this->getTableStructure($record[0]);
				if ($this->filename) $this->_html.= " DONE!\n";
			}
			if ( $this->tableData) {
				if ($this->filename) $this->_html.= "Exporting table data for '".$record[0]."': ";
				$this->getTableData($record[0],$hexValue);
				if ($this->filename) $this->_html.= " DONE!\n";
			}
		}
